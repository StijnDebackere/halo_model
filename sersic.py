...
def luminosity(pp, qq):
    ...
    def lumi(rr):
        ...
        r_split = np.array_split(rr, cpus)

        # Start the different processes
        procs = []
        for i in range(cpus):
            process = multi.Process(target=lumi_multi,
                                    args=(i, r_split[i], out_q))
        ...
        result.sort()
        final = np.concatenate([item[1] for item in result], axis=-1)
