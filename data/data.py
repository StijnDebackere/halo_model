...
# allow import of halo.tools and halo.interpolators
sys.path.append('/Users/stijn/Documents/PhD/projects/halo_model_baryons/code/')
import halo.tools as tools
import halo.input.interpolators as inp_interp

import pdb

data_dir = '/Users/stijn/Documents/PhD/projects/halo_model_baryons/data/'
fig_dir = '/Users/stijn/Documents/PhD/projects/halo_model_baryons/figures/'
...
def read_croston():
    ...
    ddir = data_dir + 'data_mccarthy/gas/Croston_data/'
    ...
    # n ~ h^(1/2)
    n = [np.loadtxt(f)[:, 2]
         for idx, f in enumerate(files)]  # [cm^-3 h_70^(1/2)]
    n_err = [np.loadtxt(f)[:, 3]
             for idx, f in enumerate(files)]  # [cm^-3 h_70^(1/2)]
    ...
    m500c = tools.radius_to_mass(r500, 500 * rho_crit * 0.7**2)
...
def read_eckert():
    ddir = data_dir + 'data_mccarthy/gas/Eckert_data/'
    ...
    for idx, prof in enumerate(rho):
        ...
        mgas = np.append(mgas,
                         tools.m_h(prof[:idx_500+1], rx[idx][:idx_500+1] *
                                   r500[idx]))
        mdata2data = np.append(mdata2data,
                               (num == numdata[idx]).nonzero()[0][0])
        # print mgas[idx]
    ...
    print("Gas fraction Ours / Eckert: ", ((mgas / (m500)) / (m500gas /
                                                              (m500 * 1.3))))
...
def bin_eckert(n=20):
    ...
    for idx_m, m_bin in enumerate(np.arange(1, len(m_bins))):
        ...
        r_range = np.logspace(np.log10(r_min+0.001), np.log10(r_max-0.001),
                              n_r)
...
def fit_croston():
    ...
    m500 = tools.radius_to_mass(r500, 500 * rho_crit * 0.7**2)
...
def fit_eckert():
    ...
    for idx, prof in enumerate(rho):
        ...
        print("{:.6f}          {:.6f}".format(np.log10(m500)[idx],
                                              m500gas_fit / m500[idx]))
...
def convert_hm(r200m=True, omega_m=0.2793):
    '''
    ...
    m500cc = tools.radius_to_mass(r500c, 500 * rho_crit * 0.7**2)
    m500ce = tools.radius_to_mass(r500e, 500 * rho_crit * 0.7**2)
    ...
    # we thus need to convert our critical density as well
    r500cc = tools.mass_to_radius(m500cc, 500 * rho_crit * 0.7**2)
    r500ce = tools.mass_to_radius(m500ce, 500 * rho_crit * 0.7**2)
    ...
    with open(data_dir + 'croston_500.p', 'wb') as f:
    ...
    if r200m:
        # assume h=1, since we already give correctly scaled h=0.7 values
        # to the fit hence we do not need to rescale the mass for c_correa
        m200mc = np.array([tools.m500c_to_m200m_duffy(m, rho_crit * 0.7**2,
                                                      omega_m * rho_crit
                                                      * 0.7**2)
                           for m in m500cc])
        m200me = np.array([tools.m500c_to_m200m_duffy(m, rho_crit * 0.7**2,
                                                      omega_m * rho_crit
                                                      * 0.7**2)
                           for m in m500ce])
        r200mc = tools.mass_to_radius(m200mc,
                                      200 * omega_m * rho_crit * 0.7**2)
        r200me = tools.mass_to_radius(m200me,
                                      200 * omega_m * rho_crit * 0.7**2)
        ...
        with open(data_dir + 'croston_200.p', 'wb') as f:
            pickle.dump(data_croston, f)
...
def f_gas(m, log10mt, a, fgas_500c_max, cosmo, f_c=0.86, sigma_lnc=0.0,
    ...
    fgas_500c_mx = fgas_500c_max(f_c=f_c,
                                 sigma_lnc=sigma_lnc)(coords).reshape(m.shape)
...
def f_gas_prms(cosmo, z=0., q=50, f_c=0.86, sigma_lnc=0.0, bias=False):
    '''
    Compute best fit parameters to the f_gas(m500c) relation with both f_gas 
    and m500c assuming h=0.7
    '''
    if bias is False:
        fname = data_dir + 'data_mccarthy/gas/m500_fgas_hydrostatic.dat'
        m500_obs, f_obs = np.loadtxt(fname, unpack=True)

    else:
        fname = 'data_mccarthy/gas/m500_fgas_bias_{}_corrected.dat'.format(str(bias).replace(".", "p"))
        m500_obs, f_obs = np.loadtxt(data_dir + fname, unpack=True)
...
def f_stars(m200m, comp='all'):
    ...
    fname = data_dir + 'data_mccarthy/stars/StellarFraction-Mh.txt'
    m_h, f_stars, f_cen, f_sat = np.loadtxt(fname, unpack=True)
...
def f_stars_interp(comp='all'):
    '''
    Return the stellar fraction interpolator as a function of halo mass as
    found by Zu & Mandelbaum (2015).
...
def fit_prms(ddir, x=500, q_rc=50, q_beta=50):
    '''
    Return observational beta profile fit parameters
    '''
    if x == 500:
        with open(ddir + 'croston_500.p', 'rb') as f:
            data_c = pickle.load(f)
        with open(ddir + 'eckert_500.p', 'rb') as f:
        ...
        with open(ddir + 'croston_200.p', 'rb') as f:
            data_c = pickle.load(f)
        with open(ddir + 'eckert_200.p', 'rb') as f:
            data_e = pickle.load(f)
...
def plot_profiles_paper(prms):
    with open(data_dir + 'croston.p', 'rb') as f:
        data_c = pickle.load(f)
    with open(data_dir + 'eckert.p', 'rb') as f:
        data_e = pickle.load(f)
    ...
    cmap = plt.get_cmap('plasma', m.shape[0])
    ...
    for idx_m, m_bin in enumerate(np.arange(1, len(m_bins))):
        ...
        for r, prof in zip(rx_c[idx_in_bin], rho_c[idx_in_bin]):
            l, = ax1.plot(r, prof / rho_crit_h70, ls='-', lw=1, c=cmap(idx_m))
            lines_c.append(l)
    ...
    lc_c = LineCollection(segs, linewidths=lws, linestyles=lss, colors=cs,
                          cmap=cmap)
    lc_c.set_array(m)
    ...
    for idx_m, m_bin in enumerate(np.arange(1, len(m_bins))):
        ...
        for r, prof in zip(rx_e[idx_in_bin], rho_e[idx_in_bin]):
            l, = ax2.plot(r, prof / rho_crit_h70, ls='-', lw=1, c=cmap(idx_m))
            lines_e.append(l)
    ...
    ax2.set_title(r'Eckert+16')

    plt.savefig(fig_dir + 'obs_profiles.pdf', transparent=True,
                bbox_inches='tight')
    plt.show()
...
def plot_profiles_fit_paper():
    ...
    with open(data_dir + 'croston_500.p', 'rb') as f:
        data_hm_c = pickle.load(f)
    with open(data_dir + 'eckert_500.p', 'rb') as f:
        data_hm_e = pickle.load(f)
    ...
    for idx_m, m_bin in enumerate(np.arange(1, len(m_bins))):
        ...
        for idx, r, prof in zip(np.arange(len(r500_c))[idx_in_bin],
                                ...
                                rho_c[idx_in_bin]):
            sl = ((prof > 0) & (r <= 1.))
            fit = prof_gas_hot(r, sl, a_c[idx], b_c[idx],
                               msl_c[idx], r500_c[idx])
            l, = ax3.plot(r[1:], (prof[1:] - fit[1:]) / fit[1:], ls='-',
                          lw=0.5, c=cmap(idx_m))
            lines_c.append(l)
    ...
    for idx_m, m_bin in enumerate(np.arange(1, len(m_bins))):
        ...
        for idx, r, prof in zip(np.arange(len(r500_med))[idx_in_bin],
                                ...
                                rho_med[idx_in_bin]):
            sl = ((prof > 0) & (r <= 1.))
            fit = prof_gas_hot(r, sl, a_e[idx], b_e[idx],
                               msl_e[idx], r500_med[idx])
            l, = ax4.plot(r[1:], (prof[1:] - fit[1:]) / fit[1:], ls='-',
                          lw=0.5, c=cmap(idx_m))
            lines_e.append(l)
    ...
    plt.savefig(fig_dir + 'obs_profiles_fit.pdf', transparent=True, bbox_inches='tight')
    plt.show()
...
def plot_beta_prof_fit_prms_paper(prms):
    with open('data/croston_500.p', 'rb') as f:
        data_c = pickle.load(f)
    ...
    ax_rc.errorbar(m500c_c, rc_c, yerr=rc_c_err, lw=0, elinewidth=1,
                   marker='s',
                   label='Croston+08')
    ax_rc.errorbar(m500c_e, rc_e, yerr=rc_e_err, lw=0, elinewidth=1,
                   marker='o',
                   label='Eckert+16')
    ...
    plt.savefig(fig_dir + 'obs_rc_fit.pdf', transparent=True,
                bbox_inches='tight')
    ...
    ax_beta.errorbar(m500c_c, b_c, yerr=b_c_err, lw=0, elinewidth=1,
                     marker='s',
                     label='Croston+08')
    ax_beta.errorbar(m500c_e, b_e, yerr=b_e_err, lw=0, elinewidth=1,
                     marker='o',
                     label='Eckert+16')
    ...
    plt.savefig(fig_dir + 'obs_beta_fit.pdf', transparent=True,
                bbox_inches='tight')
...
def plot_masses_hist_paper():
    ...
    plt.savefig(fig_dir + 'obs_masses_hist_stacked.pdf',
