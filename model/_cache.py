...
def cached_property(*parents):
    ...
    def cache(f):
        ...
        def _get_property(self):
            ...
            # If this quantity isn't in recalc, we need to construct an entry
            # for it.
            if name not in getattr(self, recalc):
                final = set()
                # For each of its parents, either get all its already known
                # parameters or just add the parent to a list (in this case
                # it's a parameter itself).
                for p in parents:
